...
object Async {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = {
    return eventuallyX map(value => if(value % 2 == 0) true else false);
  }
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = {
    return eventuallyX.map(valor => valor).recover {
      case _ => -1
    };
  }
  ...
  ): Future[(A, B)] = makeAsyncComputation1().flatMap(v1 => makeAsyncComputation2().map(v2 => (v1, v2)))
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    return makeAsyncComputation().recoverWith {
